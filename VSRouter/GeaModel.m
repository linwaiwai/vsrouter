...
@implementation NSObject( GeaModel)

+ (id)modelWithJson:(NSDictionary *)json {
    NSObject<GeaModel>  *model = [[self alloc] init];
    [model loadFromJson:json];
    
    if ([model respondsToSelector:@selector(loadJson:)]){
        [model loadJson:json];
    }
...
- (void)writeProperty:(NSString *)prop toJson:(NSMutableDictionary *)json {
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            ...
            for (id v in value) {
                ...
                if (![v conformsToProtocol:@protocol(GeaModel)]) {
                    flag = YES;
                    break;
                }
                NSMutableDictionary *dict = [NSMutableDictionary dictionary];
                NSObject<GeaModel> *model = v;
        ...
        } else if ([value conformsToProtocol:@protocol(GeaModel)]) {
            NSMutableDictionary *dict = [NSMutableDictionary dictionary];
            NSObject<GeaModel>  *model = value;
            [model writeToJson:dict];
